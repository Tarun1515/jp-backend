/*==============================================================================
  jp_sso — 020_t_sso_error_log.sql

  Centralised error log.

  One per database, deliberately. jp_sso must stay independently deployable
...
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 't_sso_error_log' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    PRINT '    Creating table [t_sso_error_log] ...';

    CREATE TABLE dbo.t_sso_error_log
    (
        ...
        UserName        sysname         NOT NULL CONSTRAINT DF_t_sso_error_log_UserName DEFAULT (SUSER_SNAME()),
        ...
        OccurredOn      datetime2       NOT NULL CONSTRAINT DF_t_sso_error_log_OccurredOn DEFAULT (SYSUTCDATETIME()),

        -- ---- standard columns (decision 2.4 — no exceptions) ---------------
        Is_Active       tinyint         NOT NULL CONSTRAINT DF_t_sso_error_log_Is_Active  DEFAULT (1),
        Is_Deleted      tinyint         NOT NULL CONSTRAINT DF_t_sso_error_log_Is_Deleted DEFAULT (0),
        CreatedOn       datetime2       NOT NULL CONSTRAINT DF_t_sso_error_log_CreatedOn  DEFAULT (SYSUTCDATETIME()),
        CreatedBy       bigint          NULL,
        ModifiedOn      datetime2       NULL,
        ModifiedBy      bigint          NULL,

        CONSTRAINT PK_t_sso_error_log PRIMARY KEY CLUSTERED (ErrorLogId),
        CONSTRAINT CK_t_sso_error_log_Is_Active  CHECK (Is_Active  IN (0, 1)),
        CONSTRAINT CK_t_sso_error_log_Is_Deleted CHECK (Is_Deleted IN (0, 1))
    );
END
ELSE
BEGIN
    PRINT '    Table [t_sso_error_log] already exists — skipped.';
END
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes
               WHERE name = 'IX_t_sso_error_log_OccurredOn' AND object_id = OBJECT_ID('dbo.t_sso_error_log'))
BEGIN
    PRINT '    Creating index [IX_t_sso_error_log_OccurredOn] ...';

    CREATE NONCLUSTERED INDEX IX_t_sso_error_log_OccurredOn
        ON dbo.t_sso_error_log (OccurredOn DESC)
...
------------------------------------------------------------------------------*/
IF NOT EXISTS (SELECT 1 FROM sys.indexes
               WHERE name = 'IX_t_sso_error_log_ErrorProcedure' AND object_id = OBJECT_ID('dbo.t_sso_error_log'))
BEGIN
    PRINT '    Creating index [IX_t_sso_error_log_ErrorProcedure] ...';

    CREATE NONCLUSTERED INDEX IX_t_sso_error_log_ErrorProcedure
        ON dbo.t_sso_error_log (ErrorProcedure, OccurredOn DESC)
